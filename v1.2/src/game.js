...
export class Game {
  ...
  _refreshPgPanel() {
    const betEl = $('pg-bet-val');
    if (betEl) betEl.textContent = fmtCredits(this.betPending);
    const ext = $('pg-extend');
    if (ext) {
      if (this.net && this.net.online && this.net.joined) {
        ...
        ext.textContent = `EXTEND ${votes}/${Math.max(1, here.length)}`;
      } else {
        ext.textContent = this._extendedSolo ? 'EXTENDED 2:00' : 'EXTEND → 2:00';
      }
    }
  }
