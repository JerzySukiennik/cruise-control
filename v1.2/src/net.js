...
const COUNTDOWN_MS = 60000;   // v1.2: 60s playground (room to shop); EXTEND vote adds another 60s once
...
export class Net {
  ...
  // so two tabs of the same browser count as two players. meta carries the cosmetic
  // identity (lifetime rank, title, skin) so peers can label + tint our ghost.
  async join(pid, name, color, meta = {}) {
    if (!this.online) return false;
    this.lbKey = pid;                                 // stable identity (leaderboard)
    this._meta = { lt: meta.lt || 0, title: meta.title || '', skin: meta.skin || '', trail: meta.trail || '' };
    ...
    await f.set(this._meRef, {
      name, color, level: 0, phase: 'level', lying: 0, dead: 0, paused: 0, vote: 0,
      lt: m.lt, title: m.title, skin: m.skin, trail: m.trail,
    ...
    this._hb = setInterval(() => {
      ...
      const mm = this._meta;
      if (this.joined) f.update(this._meRef, {
        t: f.serverTimestamp(), name, color,
        lt: mm.lt, title: mm.title, skin: mm.skin, trail: mm.trail
      }).catch(() => {});
    }, HEARTBEAT_MS);
  ...
  // Refresh the cosmetic identity mirrored to peers (after earning points / equipping).
  updateMeta(meta = {}) {
    this._meta = {
      lt: meta.lt || 0, title: meta.title || '',
      skin: meta.skin || '', trail: meta.trail || ''
    };
    if (this.joined) this.sendState(this._meta);
  }

  // EXTEND-to-2:00 vote: flag our own player node; host tallies during countdown.
  castVote() { if (this.joined) this.sendState({ vote: 1 }); }
  clearVote() { if (this.joined) this.sendState({ vote: 0 }); }
  ...
  _hostTick() {
    ...
    if (run.phase === 'level') {
      ...
      // EXTEND vote: if every player in the playground has voted and we haven't
      // extended yet, push the countdown out once by another 60s.
      if (!run.extended) {
        const here = act.filter(p => p.phase === 'playground');
        if (here.length > 0 && here.every(p => p.vote === 1)) {
          this._writeRun({
            level: run.level, phase: 'countdown',
            countdownEnd: (run.countdownEnd || this.now()) + COUNTDOWN_MS,
            startedAt: run.startedAt || this.now(), extended: 1
          });
          return;
        }
      }
