...
import { buildMissileMesh, applySkin, FlightModel } from './missile.js';
import { CollisionWorld } from './collision.js';
import { FlameTrail, DebrisPool, PuffPool, Shake, Flash, SpeedLines } from './effects.js';
import { LEVELS } from './levels.js';
import { PLAYGROUND } from './playground.js';
import {
  applyLoadout, rankFor, fmtCredits, findItem,
  ownedDefaults, equippedDefaults, labelPrefixFor
} from './shop.js';

const SAVE_KEY = 'cc_save_v1';
const MISSILE_R = 0.9;
const PAD_OFFSETS = [0, -6, 6];          // spawn pad x-offset by join order (solo = center)
const SOLO_COUNTDOWN = 60;               // v1.2: offline playground countdown (s); EXTEND doubles to 120
const SC_COSTS = [1500, 3000, 6000];     // second-chance price ladder within a level attempt-chain
...
const _peekR = new THREE.Vector3();     // camera lateral peek-into-turns temp
const _slFwd = new THREE.Vector3();     // speed-line spawn basis temps
const _slRight = new THREE.Vector3();
const _slUp = new THREE.Vector3();
...
export class Game {
  constructor(assets, input) {
    ...
    this.debris = new DebrisPool(this.scene, 360);   // v1.2: score-scaled win burst can reach ~360
    this.puff = new PuffPool(this.scene);
    this.shake = new Shake();
    this.flash = new Flash();
    this.speedLines = new SpeedLines(this.scene);    // v2.0 juice: anime speed streaks

    this.lowPerf = false;    // set by main.js: touch or <=4 cores — gates decorative movers
    ...
    // v1.2 economy / meta state
    this.shop = null;               // wired from main.js
    this.styleRank = null;          // D | C | B | A | S | null (from combo streak)
    this.fxDebris = null;           // win-explosion color override (null = level colors)
    this.titleOverride = null;      // equipped title suffix, or null = auto pilot rank
    this._saveDirty = false;        // batched-persist flag
    this._saveAcc = 0;
    this._scCount = 0;              // second chances used in this level attempt-chain
    this._deathPos = new THREE.Vector3();
    this._winSlowDur = 1.3;

    // v2.0 juice / control-feel state
    this.hitStop = 0;                 // punchy freeze timer on impact/target hit
    this._inWind = false;             // inside a level's ctx.windZones this frame
    this._nearMissDip = 0;            // brief timescale dip on a near miss
    this._nearMissNudge = new THREE.Vector3();
    this._ignitedThisLevel = false;   // first-ignition launch smoke/shake per level
    this._slAcc = 0;                  // speed-line spawn accumulator
    this._dustAcc = 0;                // skim-dust spawn accumulator
    this._speedRatio = 0;             // last frame's speed/maxSpeed (juice pools read this)
    this._fovBase = 72;
    this._slowBoost = 0;            // SLO-MO SURGE powerup active this level
    this._extendedSolo = false;     // solo playground extended once
    // all-in bet (per-player)
    this.betPending = 0;            // staked in the playground for the NEXT level
    this.betAmount = 0;             // amount actually on the table this level
    this.betActive = false;
    this.betClean = false;          // still no deaths this level
    ...
    this.hud = {
      ...
      credits: $('credits'), rank: $('rank'), style: $('style-rank'), allin: $('allin')
    };
  }

  setAudio(a) { this.audio = a; }

  // main.js tells us once whether this device is touch/low-core; ctx.low reads
  // it so level code can skip decorative (non-gameplay) movers on weak hardware.
  setLowPerf(v) { this.lowPerf = !!v; }

  setShop(s) { this.shop = s; }
  ...
  _loadSave() {
    ...
    const color = typeof s.color === 'number' ? s.color : 0;
    // v1.2 fields: migrate a pre-v1.2 save without crashing (missing -> sane defaults)
    const owned = this._migrateOwned(s.owned);
    const equipped = this._migrateEquipped(s.equipped, color, owned);
    const pu = s.powerups && typeof s.powerups === 'object' ? s.powerups : {};
    return {
      muted: !!s.muted,
      ...
      lifetime: typeof s.lifetime === 'number' && s.lifetime >= 0 ? s.lifetime : 0,
      owned,
      equipped,
      powerups: { kamikaze: pu.kamikaze | 0, slowmo: pu.slowmo | 0 },
      // SENS 1-10 slider (v2.0), default 5 = 1x mouse gain
      sens: (typeof s.sens === 'number' && s.sens >= 1 && s.sens <= 10) ? s.sens : 5
    };
  }

  // Merge stored owned ids with the free defaults (free items are always owned).
  _migrateOwned(stored) {
    const base = ownedDefaults();
    if (stored && typeof stored === 'object') {
      for (const cat of Object.keys(base)) {
        const arr = Array.isArray(stored[cat]) ? stored[cat] : [];
        for (const id of arr) if (!base[cat].includes(id)) base[cat].push(id);
      }
    }
    return base;
  }

  // Keep valid equipped ids, fall back to defaults; ensure the equipped item is owned.
  _migrateEquipped(stored, color, owned) {
    const def = equippedDefaults(color);
    const out = {};
    for (const cat of Object.keys(def)) {
      let id = stored && typeof stored === 'object' ? stored[cat] : undefined;
      if (typeof id !== 'string' || !findItem(cat, id)) id = def[cat];
      if (owned[cat] && !owned[cat].includes(id)) id = def[cat];
      out[cat] = id;
    }
    return out;
  }
  ...
  persist() {
    this._saveDirty = false; this._saveAcc = 0;
    try { localStorage.setItem(SAVE_KEY, JSON.stringify(this.save)); } catch (e) { /* private mode */ }
  }

  // Batched persist: economy changes flag dirty; update() flushes at most ~1/sec.
  _scheduleSave() { this._saveDirty = true; }

  // ---------- economy ----------

  // Every point earned bumps the run score, the spendable wallet, and lifetime total.
  _award(pts) {
    if (!(pts > 0)) return;
    this.score += pts;
  ...
  // Cosmetic identity mirrored to peers (rank derives from lt; title overrides it).
  _pushMeta() {
    if (this.net && this.net.joined) {
      this.net.updateMeta({
        lt: this.save.lifetime,
        title: this.titleOverride || '',
        skin: this.save.equipped.skin,
        trail: this.save.equipped.trail
      });
    }
  }

  // Called after a shop buy/equip: re-apply the loadout, persist, refresh HUD + peers.
  onLoadoutChanged() {
    applyLoadout(this, this.save);
    this.persist();
    this.syncHud();
    this._pushMeta();
  }

  // Push credits/rank/powerup readouts to the HUD + any open shop.
  syncHud() {
    if (this.hud.credits) this.hud.credits.textContent = fmtCredits(this.save.credits);
    if (this.hud.rank) this.hud.rank.textContent = rankFor(this.save.lifetime).name;
  ...
  _buildScene(def) {
    ...
    this.kamikaze = 1;
    // v2.0 per-level juice state
    this._ignitedThisLevel = false;
    this._inWind = false; this._nearMissDip = 0; this.hitStop = 0;
    this._slAcc = 0; this._dustAcc = 0; this._speedRatio = 0;
    ...
    // style meter resets with the run score
    this.styleRank = null;
    if (this.hud.style) this.hud.style.className = 'hidden';
    if (this.hud.allin) this.hud.allin.classList.add('hidden');
    // (re)apply the equipped cosmetic loadout to this freshly-built missile/scene
    applyLoadout(this, this.save);
  ...
  startLevel(i) {
    if (this.shop) this.shop.forceClose();
    this.mode = 'level';
    this.levelIndex = i;
    this._buildScene(LEVELS[i]);
    this.state = 'playing';
    this._consumePowerups();
    this._consumeBet();
  ...
  // Spend consumables queued in the shop's POWER-UPS tab for this level start.
  _consumePowerups() {
    const pu = this.save.powerups;
    if (!pu) return;
    if (pu.kamikaze > 0) { this.kamikaze = 1 + pu.kamikaze; pu.kamikaze = 0; this._updateKamiHud(); }
    if (pu.slowmo > 0) { this._slowBoost = pu.slowmo; pu.slowmo = 0; }
    else this._slowBoost = 0;
    this.persist();
  }

  // Put a staked all-in bet "on the table": deduct now, mark clean, show the marker.
  _consumeBet() {
    if (this.betPending > 0 && this.save.credits >= this.betPending) {
      this.betAmount = this.betPending;
      this.save.credits -= this.betAmount;
      this.betActive = true;
      this.betClean = true;
      this._scheduleSave();
    } else {
  ...
  enterPlayground() {
    ...
    // fresh playground cycle: reset extend + bet staging + our vote
    this._extendedSolo = false;
    this.betPending = 0;
    if (this.net && this.net.joined) this.net.clearVote();
    this._showPgPanel(true);
    this._refreshPgPanel();
    if (this.audio) this.audio.stopEngine();
    this._netSend(true);
    this._pushMeta();
  }
  ...
  // The floating pg-panel only works where there's a free cursor — on desktop
  // pointer lock hides it, so that hub lives in the ESC pause card instead
  // (see pause()). Touch devices keep the always-visible floating panel.
  _showPgPanel(show) {
    const el = $('pg-panel');
    if (el) el.classList.toggle('hidden', !show || !document.body.classList.contains('touch'));
  }

  // Refresh the bet display + extend-vote tally shown on the playground buttons
  // — mirrors the same numbers to both the touch floating panel and the
  // desktop pause-card hub, whichever is present in the DOM.
  _refreshPgPanel() {
    const extText = () => {
      if (this.net && this.net.online && this.net.joined) {
        const act = this.net.activePlayers();
        const here = act.filter(p => p.phase === 'playground');
        const votes = here.filter(p => p.vote === 1).length;
        return `EXTEND ${votes}/${Math.max(1, here.length)}`;
      }
      return this._extendedSolo ? 'EXTENDED 2:00' : 'EXTEND → 2:00';
    };
    for (const betId of ['pg-bet-val', 'pause-bet-val']) {
      const el = $(betId);
      if (el) el.textContent = fmtCredits(this.betPending);
    }
    for (const extId of ['pg-extend', 'btn-pause-extend']) {
      const el = $(extId);
      if (el) el.textContent = extText();
    }
  }

  // Adjust the pending all-in stake (delta, or 'max' / 'clear'), clamped to credits.
  setBet(op) {
    if (this.mode !== 'playground') return;
    const cr = this.save.credits;
    if (op === 'clear') this.betPending = 0;
    else if (op === 'max') this.betPending = cr;
    else this.betPending = Math.max(0, Math.min(cr, this.betPending + op));
    if (this.audio) this.audio.click();
    this._refreshPgPanel();
  }

  // EXTEND button: solo extends once instantly; MP casts a unanimous vote.
  requestExtend() {
    if (this.mode !== 'playground') return;
    if (this.net && this.net.online && this.net.joined) {
      this.net.castVote();
      if (this.audio) this.audio.click();
    } else if (!this._extendedSolo) {
      this._extendedSolo = true;
      this.soloCountdown = 120;
      if (this.audio) this.audio.confirm();
      this._popup('EXTENDED TO 2:00', true);
    }
    this._refreshPgPanel();
  }

  openShop() {
    if (this.mode === 'playground' && this.shop) this.shop.show();
  }
  ...
  startRun() {
    this.levelsCleared = 0;
    this.runActive = true;
    // fresh run: clear the second-chance ladder and any all-in stake
    this._scCount = 0;
    this.betPending = 0; this.betActive = false; this.betAmount = 0; this.betClean = false;
    this._extendedSolo = false;
    if (this.net && this.net.online && this.net.joined) {
  ...
  pause() {
    ...
    this.input.setActive(false);
    if (this.audio) this.audio.stopEngine();
    this.persist();      // flush pending economy on pause
    $('card-pause').classList.remove('hidden');
    // desktop hub: SHOP/EXTEND/bet live inside the pause card in the playground,
    // since pointer lock hides the cursor and the floating pg-panel is unreachable
    const pgSec = $('pause-pg');
    if (pgSec) pgSec.classList.toggle('hidden', this.mode !== 'playground');
    if (this.mode === 'playground') this._refreshPgPanel();
  ...
  toMenu() {
    ...
    this.persist();                 // flush any pending economy changes on the way out
    this.runActive = false;
    this.state = 'menu';
    this.mode = 'level';
    this.input.setActive(false);
  ...
  // Second chance: pay to respawn AT THE DEATH SITE, keeping run score/timer/fuel.
  secondChance() {
    if (this.state !== 'dead' || this.mode !== 'level') return;
    const cost = this._scCost();
    if (this.save.credits < cost) { if (this.audio) this.audio.thud(); return; }
    this.save.credits -= cost;
    this._scCount++;
    this.persist();

    const fl = this.flight;
    // restore a few units back along travel from the impact point, lifted slightly
    fl.pos.copy(this._deathPos);
    fl.pos.y += 2.0;
    fl.prevPos.copy(fl.pos);
    fl.speed = 0; fl.gvel = 0; fl.launched = false;   // launched=false -> the latch applies
    fl.roll = 0; fl.pitchRate = 0; fl.yawRate = 0;
    const bank = this.missileMesh.userData.bank;
    bank.rotation.set(0, 0, 0);
    this.missileMesh.visible = true;
    ...
    this.hud.crosshair.classList.add('hidden');
    this.input.setActive(true);
    this.input.consumeThrust();   // the SECOND-CHANCE press must NOT ignite the engine
    this.input.requestLock();
    if (this.audio) this.audio.confirm();
    this.syncHud();
    // betClean stays false — using a second chance means you died this level
  }

  // Show/label the SECOND CHANCE button on the death card if the player can afford it.
  _refreshDeathCard() {
    const btn = $('btn-second-chance');
    if (!btn) return;
    const cost = this._scCost();
    if (this.mode === 'level' && this.save.credits >= cost) {
      btn.textContent = `SECOND CHANCE (${fmtCredits(cost)})`;
  ...
  _knockdown(normal) {
    ...
    this.puff.burst(fl.pos, 22);   // v2.0: bigger dust ring on knockdown
    this.shake.add(0.3);
    this.flash.hit(0.1);
  ...
  // ---------- style meter (DMC-style D->C->B->A->S from the combo streak) ----------

  _styleRankFor(combo) {
    if (combo >= 11) return 'S';
    if (combo >= 7) return 'A';
    if (combo >= 4) return 'B';
    if (combo >= 2) return 'C';
    if (combo >= 1) return 'D';
  ...
  _updateStyle() {
    const r = this._styleRankFor(this.combo);
    if (r === this.styleRank) return;
    this.styleRank = r;
    const el = this.hud.style;
    if (!el) return;
    if (!r) { el.className = 'hidden'; el.textContent = ''; return; }
    el.textContent = r;
    el.className = 'sr-' + r;      // color per rank + clears .hidden
    el.style.animation = 'none';   // retrigger the pulse on change
    void el.offsetWidth;
    el.style.animation = '';
  }
  ...
  _crash() {
    ...
    this.hud.crosshair.classList.add('hidden');
    this.input.setActive(false);
    // remember the death site (pre-impact) for a possible second chance
    this._deathPos.copy(this.flight.prevPos);
    // an all-in run is no longer flawless
    if (this.betActive) { this.betClean = false; this._updateAllinHud(); }
    this.persist();      // flush earned economy on death
    if (this.audio) { this.audio.stopEngine(); this.audio.crash(); }
    const p = this.flight.pos;
    this.puff.burst(p, 16);
    // debris of the missile itself: double count, tinted by the equipped skin
    const skin = findItem('skin', this.save.equipped.skin) || findItem('skin', 'c0');
    this.debris.burst(p, 52, [skin.body, 0xd9d9de, 0xa8a8b2, 0x54586a], 0.7);
    this.shake.add(0.55);
    this.flash.hit(0.25);
    // crumpled missile
    const bank = this.missileMesh.userData.bank;
  ...
  _win() {
    if (this.state !== 'playing') return;
    this.hitStop = 0.09;   // punchy freeze on target hit before the slow-mo ramp
    this.state = 'win';
    this.winTimer = 0;
    this.aiming = false;
    if (this.dashing) this._endDash(1);
    ...
    // ---- score-scaled detonation (#19) ----
    const runScore = this.score;                       // this level's earned score (pre-bonus)
    const isS = this.styleRank === 'S';
    const count = Math.round(140 + Math.min(runScore * 0.08, 220));   // up to ~360
    const colors = this.fxDebris || this.level.targetColors;          // FX override or level palette
    ...
    // clear bonus is points too
    this._award(500);

    // ---- S-RANK strike bonus (#20) ----
    if (isS) {
    ...
    if (this.betActive) {
      if (this.betClean) {
        ...
        this.save.credits += payout;                   // gambling winnings (credits only)
        this._scheduleSave();
        this._popup('ALL-IN WON! +' + payout, true);
      }
      this.betActive = false; this.betAmount = 0; this.betClean = false;
      this._updateAllinHud();
    }
    ...
    this._scCount = 0;                                  // level cleared -> reset SC ladder
    this.levelsCleared++;
    if (this.levelsCleared > this.save.bestRun) this.save.bestRun = this.levelsCleared;
    this.persist();
    this.syncHud();
    if (this.net && this.net.joined) {
  ...
  _updateAllinHud() {
    const el = this.hud.allin;
    if (!el) return;
    if (!this.betActive) { el.classList.add('hidden'); return; }
    el.classList.remove('hidden');
    el.classList.toggle('lost', !this.betClean);
    el.textContent = this.betClean
      ? `ALL-IN ${fmtCredits(this.betAmount)} — NO DEATHS`
      : `ALL-IN ${fmtCredits(this.betAmount)} — LOST`;
  }

  // ---------- breakables / toys ----------

  _breakWall(c) {
    this.hitStop = 0.04;   // small punch on a wall break
  ...
  update(dtReal) {
    ...
    // batched persist: flush economy changes at most ~1/sec
    if (this._saveDirty) {
      this._saveAcc += dtReal;
      if (this._saveAcc >= 1) this.persist();
    }

    // hit-stop: a punchy freeze on target hit / wall break — gameplay logic is
    // skipped for the duration but shake/flash keep decaying so the impact
    // still reads immediately, then the world un-freezes into the slow-mo ramp
    if (this.hitStop > 0) {
      this.hitStop -= dtReal;
      return;
    }
    ...
    if (this.state === 'complete') {
      ...
      this.speedLines.update(dtReal, 0);
      return;
    }

    // shop open in the playground: countdown/ghosts/effects keep running, missile frozen
    if (this.shop && this.shop.isOpen()) {
      if (this.mode === 'playground') this._updatePlayground(dtReal);
      if (this.state === 'menu' || this.state === 'complete') return; // countdown may transition out
    ...
    // timescale
    let targetTs = 1;
    const slowDrain = 0.4 * (this._slowBoost ? 0.5 : 1);   // SLO-MO SURGE powerup drains slower
    if (this.state === 'playing' && this.aiming) {
      targetTs = 0.25;
    } else if (this.state === 'playing' && this.input.slowmo && this.slowMeter > 0 && this.flight.launched) {
      targetTs = 0.35;
      this.slowMeter = Math.max(0, this.slowMeter - dtReal * slowDrain);
    } else if (this.state === 'playing') {
      this.slowMeter = Math.min(1, this.slowMeter + dtReal * 0.16);
    }
    if (this.state === 'win' && this.winTimer < this._winSlowDur) targetTs = 0.22;
    // death: 300ms slow-mo crawl before the death card timing takes over
    if (this.state === 'dead' && this.deathTimer < 0.3) targetTs = 0.15;
    // near-miss feedback: a brief timescale dip, decays on its own timer
    if (this._nearMissDip > 0) {
      targetTs = Math.min(targetTs, 0.85);
      this._nearMissDip = Math.max(0, this._nearMissDip - dtReal);
    }
    ...
    if (this.state === 'playing') this._updatePlaying(dt, dtReal);
      ...
      if (this.deathTimer > 0.55 && $('card-death').classList.contains('hidden')) {
        ...
        this._refreshDeathCard();
      }
    } else if (this.state === 'win') {
      this.winTimer += dtReal;
      if (this.winTimer > Math.max(2.5, this._winSlowDur + 0.6) && this.runActive) this.enterPlayground();
    }
    ...
    this.puff.update(dt);
    this.speedLines.update(dtReal, this._speedRatio);
    this._updateCamera(dtReal);
  }
  ...
  _updatePlaying(dt, dtReal) {
    ...
    const fl = this.flight;
    const steer = (this.aiming || this.dashing) ? ZERO_STEER : input.getSteer(dtReal);
    ...
    // wind zones (SLIPSTREAM): axis-aligned box test, cheap — no rotation needed.
    // Inside the band the missile gets a speed boost; game._updateFov reads
    // this._inWind for the matching FOV kick.
    this._inWind = false;
    if (fl.launched && this.ctx.windZones && this.ctx.windZones.length) {
      const p = fl.pos;
      for (const z of this.ctx.windZones) {
        if (p.y >= z.y0 && p.y <= z.y1 && Math.abs(p.x - z.cx) <= z.hx && Math.abs(p.z - z.cz) <= z.hz) {
          this._inWind = true;
          const target = fl.params.maxSpeed * z.mul;
          if (fl.speed < target) fl.speed = Math.min(target, fl.speed + fl.params.accel * 1.5 * dt);
          break;
        }
      }
    }
    ...
    // launch juice: pad smoke + a bigger shake on the FIRST ignition this level
    if (!wasLaunched && fl.launched) {
      if (!this._ignitedThisLevel) {
        this._ignitedThisLevel = true;
        this.puff.burst(fl.pos, 26);
        this.shake.add(0.9);
      } else {
        this.shake.add(0.15);
      }
    ...
    if (fl.launched) {
      ...
      // flame trail — thickens (rate + particle size) with the speed ratio
      this._speedRatio = Math.min(1, fl.speed / fl.params.maxSpeed);
      if (thrustEff) {
        this.flameAcc += dt;
        fl.forward(_f);
      ...
      // speed lines: anime-style streaks once past 80% of max speed
      if (this._speedRatio > 0.8) {
        this._slAcc += dt;
        while (this._slAcc >= 0.035) {
          this._slAcc -= 0.035;
          _slFwd.set(0, 0, -1).applyQuaternion(this.camera.quaternion);
          _slRight.set(1, 0, 0).applyQuaternion(this.camera.quaternion);
          _slUp.set(0, 1, 0).applyQuaternion(this.camera.quaternion);
          this.speedLines.spawn(this.camera.position, _slFwd, _slRight, _slUp);
        }
      } else {
        this._slAcc = 0;
      }
      ...
      if (this.graceT <= 0) {
        ...
        for (const c of res.nearMisses) {
          ...
          // near-miss juice: brief timescale dip + a small camera nudge away
          // from the obstacle we just cleared (decayed each frame in update())
          this._nearMissDip = 0.06;
          this.world.normalAt(c, fl.pos, _n);
          this._nearMissNudge.copy(_n);
        }
      }
      ...
      // skim dust: small puffs while hugging the ground fast (ROOF RUSH's
      // visual counterpart — reads the speed even when the trick isn't firing)
      if (thrustEff && alt < 1.2 && fl.speed > fl.params.maxSpeed * 0.4) {
        this._dustAcc += dt;
        if (this._dustAcc > 0.08) { this._dustAcc = 0; this.puff.burst(fl.pos, 3); }
      }
      ...
      if (this.comboTimer > 0) {
        this.comboTimer -= dt;
        if (this.comboTimer <= 0) { this.combo = 0; this._updateStyle(); }
      }
  ...
  _updateAim(dtReal) {
    const steer = this.input.getSteer(dtReal);
  ...
  _updatePlayground(dtReal) {
    ...
    // desktop has no free cursor in the playground (pointer lock) — point at
    // the ESC pause card, which is where SHOP/EXTEND/bet live there
    if (!document.body.classList.contains('touch')) txt += '  ·  ESC — SHOP / EXTEND / BET';
    if (txt !== this.pgText) {
      this.pgText = txt;
      this.hud.pg.textContent = txt;
    }

    // keep the bet display + extend-vote tally fresh
    this._refreshPgPanel();
  ...
  _makeGhost(peer) {
    ...
    // apply the peer's shop skin BEFORE cloning so the colors carry into the ghost mats
    const skin = peer.skin ? findItem('skin', peer.skin) : null;
    if (skin) applySkin(mesh, skin);
    mesh.traverse(c => {
      if (c.isMesh) {
    ...
    this.scene.add(mesh);
    // name label: RANK/TITLE prefix on top, pilot name below
    const cv = document.createElement('canvas');
    cv.width = 256; cv.height = 80;
    const g = cv.getContext('2d');
    g.textAlign = 'center'; g.textBaseline = 'middle';
    g.lineJoin = 'round';
    const prefix = labelPrefixFor(peer.lt || 0, peer.title || '').slice(0, 12);
    const nm = String(peer.name || '???').slice(0, 10);
    g.font = '800 22px -apple-system, "Segoe UI", Arial, sans-serif';
    g.lineWidth = 6; g.strokeStyle = '#14141f';
    g.strokeText(prefix, 128, 18); g.fillStyle = '#ffd23f'; g.fillText(prefix, 128, 18);
    g.font = '900 38px -apple-system, "Segoe UI", Arial, sans-serif';
    g.lineWidth = 8; g.strokeStyle = '#14141f';
    g.strokeText(nm, 128, 52); g.fillStyle = '#ffffff'; g.fillText(nm, 128, 52);
    const tex = new THREE.CanvasTexture(cv);
    tex.colorSpace = THREE.SRGBColorSpace;
    const label = new THREE.Sprite(new THREE.SpriteMaterial({ map: tex, transparent: true }));
    label.scale.set(7, 2.19, 1);
  ...
  _updateHud(force = false) {
    ...
    if (h.credits) h.credits.textContent = fmtCredits(this.save.credits);
    if (h.rank) h.rank.textContent = rankFor(this.save.lifetime).name;
    if (this.combo > 1) {
  ...
  // FOV kick (v2.0 juice): base 72, +6 at full throttle, +10 during kamikaze
  // dash / inside a wind zone. updateProjectionMatrix is only called when the
  // fov actually moved a meaningful amount, to avoid a matrix rebuild every frame.
  _updateFov(dtReal) {
    let target = this._fovBase;
    if (this.state === 'playing' && this.flight.launched) {
      target += 6 * this.throttleSm;
      if (this.dashing) target += 10;
      if (this._inWind) target += 10;
    }
    const cur = this.camera.fov;
    const next = cur + (target - cur) * Math.min(1, dtReal * 6);
  ...
  _updateCamera(dtReal) {
    ...
    // control feel: less chase lag at high speed so quick turns don't leave
    // the camera trailing behind
    const baseK = fl.speed > fl.params.maxSpeed * 0.7 ? 9 : 7;
    const k = 1 - Math.exp(-dtReal * (this.state === 'dead' || this.state === 'lying' ? 1.5 : baseK));
    ...
    // control feel: slight lateral peek into turns, opposite the yaw steer
    if (this.state === 'playing') {
      const peek = Math.max(-1.2, Math.min(1.2, -fl.yawRate * 0.6));
      if (peek) {
        _peekR.set(1, 0, 0).applyQuaternion(fl.quat);
        this.camera.position.addScaledVector(_peekR, peek);
      }
    }
    // near-miss camera nudge away from the obstacle we just cleared, decaying
    if (this._nearMissDip > 0) {
      this.camera.position.addScaledVector(this._nearMissNudge, 0.4 * (this._nearMissDip / 0.06));
    }
    this.shake.apply(this.camera);
  }
}
