...
export function buildMissileMesh(tint = 0xf2f2f0) {
  ...
  // body + nose are separate materials so shop skins can go two-tone (e.g. neon)
  const white = new THREE.MeshLambertMaterial({ color: tint });
  const noseMat = new THREE.MeshLambertMaterial({ color: tint });
  ...
  // primary tint (net color / lobby color) = body + nose; mats exposes each part for shop skins
  group.userData.tintMats = [white, noseMat];
  group.userData.mats = { body: white, nose: noseMat, fin: grey, nozzle: dark, dot: red };
  return group;
}

// Apply a shop skin definition to a missile mesh: body/nose/accent colors, optional
// emissive glow (neon/gold/chrome), and an optional chunkier body scale (retro).
export function applySkin(mesh, skin) {
  if (!mesh || !mesh.userData || !skin) return;
  const m = mesh.userData.mats;
  const body = skin.body !== undefined ? skin.body : 0xf2f2f0;
  const nose = skin.nose !== undefined ? skin.nose : body;
  if (m) {
    m.body.color.setHex(body);
    m.nose.color.setHex(nose);
  ...
  if (bank) {
    const s = skin.shape;
    if (Array.isArray(s)) bank.scale.set(s[0], s[1], s[2] !== undefined ? s[2] : 1);
    else bank.scale.set(1, 1, 1);
  }
}
...
export class FlightModel {
  ...
  update(dt, steer, thrust) {
    ...
    // steering authority needs airflow — smoothstep curve (control feel v2.0):
    // low speed steering is less mushy but top speed doesn't oversteer, vs. the
    // old straight-line ramp which felt linear-mushy at low speed
    const sN = Math.max(0, Math.min(1, this.speed / (p.refSpeed * 1.2)));
    const smooth = sN * sN * (3 - 2 * sN);
    const authority = 0.25 + 0.75 * smooth;
