...
export class Input {
  constructor(canvas, isTouch) {
    ...
    this.aimX = 0; this.aimY = 0;      // virtual stick [-1,1] (smoothed, pointer-lock)
    this._rawAimX = 0; this._rawAimY = 0; // raw pointer-lock accumulator (pre-smoothing)
    this.gain = 1;                      // sensitivity multiplier, see setSensitivity()
    ...
    this.onSecondChance = null;         // C key (only acts on the death card)
    this._keys = new Set();
    this._joyId = null;
  ...
  _bind() {
    window.addEventListener('keydown', e => {
      ...
      if (k === 'KeyC' && this.onSecondChance) this.onSecondChance();
    ...
    if (!this.isTouch) {
      ...
      window.addEventListener('mousemove', e => {
        if (this.locked) {
          this._rawAimX += e.movementX * 0.0026 * this.gain;
          this._rawAimY -= e.movementY * 0.0026 * this.gain;
          const m = Math.hypot(this._rawAimX, this._rawAimY);
          if (m > 1) { this._rawAimX /= m; this._rawAimY /= m; }
        } else {
          const cx = window.innerWidth / 2, cy = window.innerHeight / 2;
          const s = Math.min(window.innerWidth, window.innerHeight) * 0.5;
          this.mouseNX = Math.max(-1, Math.min(1, (e.clientX - cx) / s * this.gain));
          this.mouseNY = Math.max(-1, Math.min(1, -(e.clientY - cy) / s * this.gain));
        }
      });
  ...
  _bindTouch() {
    ...
    const joyMove = t => {
      ...
      // touch joystick gets the sensitivity gain mildly (full gain feels twitchy on a stick)
      const g = 1 + (this.gain - 1) * 0.4;
      this.aimX = Math.max(-1, Math.min(1, (dx / R) * g));
      this.aimY = Math.max(-1, Math.min(1, (-dy / R) * g));
    };
  ...
  resetAim() {
    this.aimX = 0; this.aimY = 0; this._rawAimX = 0; this._rawAimY = 0;
    this.mouseNX = 0; this.mouseNY = 0;
  }

  // SENS 1-10 slider (default 5 = 1x gain), range 0.5x-2x — see main.js wiring.
  setSensitivity(level) {
    const t = Math.max(1, Math.min(10, level));
    this.gain = t <= 5 ? 0.5 + (t - 1) * 0.125 : 1 + (t - 5) * 0.2;
  }

  // dt drives the pointer-lock aim smoothing (~25/s exp filter) so raw mouse
  // jitter doesn't translate 1:1 into steering — crisp but not twitchy.
  getSteer(dt = 1 / 60) {
    if (this.locked) {
      const k = 1 - Math.exp(-dt * 25);
      this.aimX += (this._rawAimX - this.aimX) * k;
      this.aimY += (this._rawAimY - this.aimY) * k;
    }
    let x, y;
