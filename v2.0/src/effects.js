...
export class FlameTrail {
  ...
  spawn(pos, back, missileSpeed, speedRatio = 0) {
    ...
    this.pool.parts[i] = {
      ...
      size: (0.5 + Math.random() * 0.3) * sizeMul,
      seed: Math.random(),                      // per-particle hue offset / sparkle roll
      life, maxLife: life
    };
  }
  update(dt) {
    if (this.mode === 'rainbow') this.hue = (this.hue + dt * 0.4) % 1;
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      // base 3-stop gradient, then per-mode overrides
      if (t < 0.45) _c.copy(this.c0).lerp(this.c1, t / 0.45);
      else _c.copy(this.c1).lerp(this.c2, (t - 0.45) / 0.55);
      if (this.mode === 'rainbow') _c.setHSL((this.hue + pt.seed) % 1, 0.95, 0.55 * (1 - t * 0.5));
      else if (this.mode === 'stars' && pt.seed < 0.3) _c.setRGB(1, 1, 1); // twinkling specks
...
// Anime-style speed streaks at the screen edges, above ~80% max speed. Reuses
// the CubePool pattern (chunky cubes, no alpha-heavy sprites) — cheap, capped.
export class SpeedLines {
  constructor(scene) {
    this.pool = new CubePool(scene, 40, new THREE.MeshBasicMaterial({
      color: 0x0a0a12, transparent: true, opacity: 0.5, depthWrite: false
    }));
  }
  // camPos: Vector3, fwd/right/up: unit Vector3 camera-space basis vectors
  spawn(camPos, fwd, right, up) {
    const i = this.pool.alloc();
    const side = Math.random() < 0.5 ? -1 : 1;
    const ox = side * (6 + Math.random() * 5);
    const oy = (Math.random() - 0.5) * 7;
    const life = 0.2 + Math.random() * 0.1;
    _q.setFromUnitVectors(Z_AXIS, _dir.copy(fwd));
    this.pool.parts[i] = {
      px: camPos.x + right.x * ox + up.x * oy + fwd.x * 10,
      py: camPos.y + right.y * ox + up.y * oy + fwd.y * 10,
      pz: camPos.z + right.z * ox + up.z * oy + fwd.z * 10,
  ...
  update(dt, speedRatio = 0) {
    const { parts, mesh } = this.pool;
    const spd = 30 + speedRatio * 70;
    const len = 3 + speedRatio * 5;
    for (let i = 0; i < parts.length; i++) {
      const pt = parts[i];
      if (!pt) continue;
      pt.life -= dt;
      if (pt.life <= 0) { parts[i] = null; mesh.setMatrixAt(i, HIDE); continue; }
      pt.px += pt.fx * spd * dt; pt.py += pt.fy * spd * dt; pt.pz += pt.fz * spd * dt;
      const t = 1 - pt.life / pt.maxLife;
      const sc = (1 - t * 0.4);
      _q.set(pt.qx, pt.qy, pt.qz, pt.qw);
      _m.compose(_p.set(pt.px, pt.py, pt.pz), _q, _s.set(0.15, 0.15, len * sc));
      mesh.setMatrixAt(i, _m);
    }
    mesh.instanceMatrix.needsUpdate = true;
  }
...
export class Flash {
  ...
  // color lets the S-RANK strike tint the screen flash gold instead of white
  hit(strength = 1, color = '#ffffff') { this.v = Math.min(1, strength); this.el.style.background = color; }
  update(dt) {
    if (this.v <= 0) return;
