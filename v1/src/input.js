...
export class Input {
  ...
  _bind() {
    window.addEventListener('keydown', e => {
      ...
      const k = e.code;
      this._keys.add(k);
      if (k === 'Space') { this.thrust = true; e.preventDefault(); }
    ...
    if (!this.isTouch) {
      ...
      window.addEventListener('mousemove', e => {
        if (this.locked) {
          this.aimX += e.movementX * 0.0045;
          this.aimY -= e.movementY * 0.0045;
          const m = Math.hypot(this.aimX, this.aimY);
          if (m > 1) { this.aimX /= m; this.aimY /= m; }
        } else {
          const cx = window.innerWidth / 2, cy = window.innerHeight / 2;
          const s = Math.min(window.innerWidth, window.innerHeight) * 0.35;
  ...
  _bindTouch() {
    ...
    hookBtn(document.getElementById('btn-thrust'), this._thrustIds, v => { this.thrust = v; });
